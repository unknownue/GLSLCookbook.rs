...
pub fn load_texture(display: &impl Facade, path: impl AsRef<Path>) -> GLResult<Texture2d> {
    ...
    // The original C++ implementation does not use mipmap, here use mipmap should generally ok.
    let texture = Texture2d::with_mipmaps(display, raw_image, glium::texture::MipmapsOption::AutoGeneratedMipmaps)
