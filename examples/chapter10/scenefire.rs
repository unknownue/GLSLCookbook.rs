...
impl Scene for SceneFire {

    fn new(display: &impl Facade) -> GLResult<SceneFire> {

        // Shader Program ------------------------------------------------------------
        let programs = SceneFire::compile_shader_program(display)
            .map_err(GLErrorKind::CreateProgram)?;
        let flat_program = SceneFire::compile_flat_shader_program(display)
            .map_err(GLErrorKind::CreateProgram)?;
        // ----------------------------------------------------------------------------
        ...
        let (vbuffer1, vbuffer2) = SceneFire::init_buffers(display, PARTICLE_LIFETIME, N_PARTICLES)?;
        // ----------------------------------------------------------------------------

        // Initialize Texture ---------------------------------------------------------
        let fire_tex = load_texture(display, "media/texture/fire.png")?;
        let random_tex = particle::random_tex_1d(display, N_PARTICLES * 2)?;
        // ----------------------------------------------------------------------------
...
impl SceneFire {
    ...
    fn compile_shader_program(display: &impl Facade) -> Result<[Program; 2], ProgramCreationError> {
        ...
        let pass1_vertex   = include_str!("shaders/fire/pass1.vert.glsl");
        let pass1_fragment = include_str!("shaders/fire/pass1.frag.glsl");

        let pass2_vertex   = include_str!("shaders/fire/pass2.vert.glsl");
        let pass2_fragment = include_str!("shaders/fire/pass2.frag.glsl");
