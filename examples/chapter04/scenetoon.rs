...
use cookbook::objects::{Teapot, Plane, Torus};
...
impl Scene for SceneToon {

    fn new(display: &impl Facade) -> GLResult<SceneToon> {

        // Shader Program ------------------------------------------------------------
        let program = SceneToon::compile_shader_program(display)
        ...
        let teapot_material = MaterialInfo {
            ...
            Kd: [0.9_f32, 0.5, 0.3], ..Default::default()
        };
        let plane = Plane::new(display, 50.0, 50.0, 1, 1, 1.0, 1.0)?;
        let plane_material = MaterialInfo {
            Ka: [0.2_f32, 0.2, 0.2],
            Kd: [0.7_f32, 0.7, 0.7], ..Default::default()
        };
        // ----------------------------------------------------------------------------


        // Initialize MVP -------------------------------------------------------------
        let view = Mat4F::look_at_rh(Vec3F::new(4.0, 4.0, 6.4), Vec3F::new(0.0, 0.75, 0.0), Vec3F::unit_y());
...
impl SceneToon {
    ...
    fn compile_shader_program(display: &impl Facade) -> Result<Program, ProgramCreationError> {
        ...
        let vertex_shader_code   = include_str!("shaders/toon.vert.glsl");
        let fragment_shader_code = include_str!("shaders/toon.frag.glsl");
